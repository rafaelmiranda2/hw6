...
public:
    ...
    HASH_INDEX_T next() 
    {
      ...
      HASH_INDEX_T currLoc = (this->start_ + this->numProbes_ * dhstep_) % this->m_;
      this->numProbes_++;
      return currLoc;
    }
};
...
HashTable<K,V,Prober,Hash,KEqual>::~HashTable()
{
  for (size_t i = 0; i < table_.size(); i++) { //Clear out table
    delete table_[i];
  }
}
...
{
  double hashFactor = static_cast<double>(size_) / table_.size();
  if (hashFactor >= resizeAlpha_) { //If hashFactor exceeds our threshold, resize the table
    resize();
  }
  HASH_INDEX_T currLoc = probe(p.first);
  if (currLoc == npos) {
    throw::std::logic_error("Full Hashtable");
  }
  if (table_[currLoc] == nullptr) {
    table_[currLoc] = new HashItem(p); //Insert if space is available
    numValues++; //Update important values
    size_++;
  }
  else {
    table_[currLoc]->item.second = p.second; //Update value
    if (table_[currLoc]->deleted == true) { //If item was deleted, make it active again, else update its value normally
      table_[currLoc]->deleted = false;
      table_[currLoc]->item = p;
      numValues++;
      size_++;
    }
    else {
      table_[currLoc]->item.second = p.second;
    }
  }
}
...
{
  HASH_INDEX_T currLoc = probe(key);
  if (currLoc != npos && table_[currLoc] != nullptr && table_[currLoc]->deleted == false) { //Mark as deleted, update values
    table_[currLoc]->deleted = true;
    numValues--;
    size_--;
    numDeleted++;
  }
...
{
  static const size_t CAPACITIES_SIZE = sizeof(CAPACITIES) / sizeof(CAPACITIES[0]);
  ...
  for (size_t i = 0; i < tempTable.size(); i++) {
    HashItem* currItem = tempTable[i];
    if (currItem != nullptr && currItem->deleted == false) {
      HASH_INDEX_T tempLoc = probe(currItem->item.first);
      table_[tempLoc] = currItem;
      numValues++;
    }
    else {
      delete currItem;
